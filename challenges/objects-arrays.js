...
const universities = [];
const universitites = graduates.map(function(item){
 ...
 const unisWithUni = []; 
  for(let i=0; i < graduates.length; i++){
    if(graduates.includes('Uni')){
      unisWithUni.push(unisWithUni[i]);
      }
     }
...
const lowCaseAnimalNames = zooAnimals.map(function(item, animal){
  return item.animal.lowerCase();
});
